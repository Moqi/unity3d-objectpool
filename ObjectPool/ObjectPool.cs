...
ObjectPool
{
    ...
    /// </param>
    public
    ObjectPool(GameObject prefab, ushort capacity, MonoBehaviour thisMonobehaviour = null)
    {
        objectPrefab = prefab;
    ...
    /// </param>
    public GameObject
    Create(Vector3 position, float lifeTime = float.PositiveInfinity)
    {
        return Create(position, Quaternion.identity, lifeTime);
    }
    ...
    /// </param>
    public GameObject
    Create(Vector3 position, Quaternion rotation, float lifeTime = float.PositiveInfinity)
    {
        //Debug.Log("Creating object @ " + position + " with lifetime " + lifeTime);  
        ...
        else
        {
            ...
            returnObjectTransform.position = position;
            returnObjectTransform.rotation = rotation;
    ...
    Destroy(GameObject objectReference)
    {
        ...
        // already inactive, counts as destroyed
        if (false == objectReference.activeInHierarchy)
        {
            Debug.LogWarning(objectReference.name + " instance already inactive.");
            return;
        }
        else // check that it's an actual pooled object
        {
            int _objectIndex = poolActive.IndexOf(objectReference);

            if (_objectIndex == -1)
            {
                Debug.LogWarning(objectReference.name + " instance not found in " + objectPrefab.name + " pool.");
    ...
    DestroyAll()
    {
        ...
        for (uint i = 0; poolActive.Count != i; i++)
        {
            _objectReference = (GameObject)poolActive[0];
